...
let _url =
  "https://generativelanguage.googleapis.com/v1beta2/models/text-bison-001:generateText?key=AIzaSyBvQ8j8TDPTTqnRU7pgQZeDeqfFSLjFniA";
...
const ChatbotView = () => {
  ...
  async function onSubmit(event) {
    ...
    if (json.hasOwnProperty("candidates")) {
      ...
      setKeywordOnResponse(keyword);
      setResponse(result);
      setKeyword("");
    } else if (json.hasOwnProperty("error")) {
      setResponse("Jawaban tidak ditemukan!");
    }
  }

  return (
    <>
      <MainLayout pageTitle="Chatbot">
        <section
          id="chatbot"
          className="w-full py-[120px] flex flex-col justify-center"
        >
          <h1 className="text-xl text-pgray font-bold text-center mb-10">
            Chatbot
          </h1>
          <div className="chat chat-start">
            <div className="chat-bubble">
              <p className="">
                <b>{keywordOnResponse}:</b> {response}
              </p>
            </div>
          </div>
          <form
